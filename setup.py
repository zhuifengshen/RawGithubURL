...
with io.open(os.path.join(here, 'github', '__about__.py'), encoding='utf-8') as f:  # custom
    exec(f.read(), about)
...
class PyPiCommand(Command):
    ...
    def run(self):
        ...
        try:
            ...
            rmtree(os.path.join(here, 'github.egg-info'))  # custom
        except OSError:
            pass
...
setup(
    ...
    packages=find_packages(exclude=['tests', 'test.*', 'images']),  # custom
    package_data={  # custom
        '': ['README.md'],
    },
